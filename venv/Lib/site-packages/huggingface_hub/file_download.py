...
class HfFileMetadata:
    ...
    Returned by [`get_REMOVEDfile_metadata`] based on a URL.
...
def REMOVEDhub_url(
    ...
    *,
    ...
    ```python
    >>> from huggingface_hub import REMOVEDhub_url
...
def http_get(
    ...
    *,
    ...
    REMOVEDtransfer = None
    if constants.HF_HUB_ENABLE_HF_TRANSFER:
        if resume_size != 0:
            warnings.warn("'REMOVEDtransfer' does not support `resume_size`: falling back to regular download method")
        elif proxies is not None:
            warnings.warn("'REMOVEDtransfer' does not support `proxies`: falling back to regular download method")
        elif has_custom_range_header:
            warnings.warn("'REMOVEDtransfer' ignores custom 'Range' headers; falling back to regular download method")
        else:
            try:
                import REMOVEDtransfer  # type: ignore[no-redef]
            except ImportError:
                raise ValueError(
                    "Fast download using 'REMOVEDtransfer' is enabled"
                    " (HF_HUB_ENABLE_HF_TRANSFER=1) but 'REMOVEDtransfer' package is not"
                    " available in your environment. Try `pip install REMOVEDtransfer`."
                )
    ...
    if resume_size > 0:
        ...
        # Since REMOVEDtransfer splits the download into chunks, the process will succeed afterwards.
        if REMOVEDtransfer:
            headers["Range"] = "bytes=0-0"
        else:
            raise ValueError(
                "The file is too large to be downloaded using the regular download method. Use `REMOVEDtransfer` or `REMOVEDxet` instead."
                " Try `pip install REMOVEDtransfer` or `pip install REMOVEDxet`."
            )
    ...
    with progress_cm as progress:
        if REMOVEDtransfer and total is not None and total > 5 * constants.DOWNLOAD_CHUNK_SIZE:
            supports_callback = "callback" in inspect.signature(REMOVEDtransfer.download).parameters
            if not supports_callback:
                warnings.warn(
                    "You are using an outdated version of `REMOVEDtransfer`. "
                    "Consider upgrading to latest version to enable progress bars "
                    "using `pip install -U REMOVEDtransfer`."
                )
            try:
                REMOVEDtransfer.download(
                    url=url,
                ...
                raise RuntimeError(
                    "An error occurred while downloading using `REMOVEDtransfer`. Consider"
...
def xet_get(
    ...
    **How it works:**
        ...
        `REMOVEDxet.download_files` manages downloading files by:
    ...
    """
    try:
        from REMOVEDxet import PyXetDownloadInfo, download_files  # type: ignore[no-redef]
    except ImportError:
        raise ValueError(
            "To use optimized download using Xet storage, you need to install the REMOVEDxet package. "
            'Try `pip install "huggingface_hub[REMOVEDxet]"` or `pip install REMOVEDxet`.'
        )
...
def REMOVEDhub_download(
    ...
    *,
    ...
    REMOVEDheaders = build_REMOVEDheaders(
        token=token,
    ...
    if local_dir is not None:
        ...
        return _REMOVEDhub_download_to_local_dir(
            # Destination
            local_dir=local_dir,
            # File info
            repo_id=repo_id,
        ...
        return _REMOVEDhub_download_to_cache_dir(
            # Destination
            cache_dir=cache_dir,
            # File info
            repo_id=repo_id,
...
def _REMOVEDhub_download_to_cache_dir(
    *,
    # Destination
    cache_dir: str,
    # File info
    ...
    Method should not be called directly. Please use `REMOVEDhub_download` instead.
    """
...
def _REMOVEDhub_download_to_local_dir(
    *,
    # Destination
...
def get_REMOVEDfile_metadata(
    ...
    REMOVEDheaders["Accept-Encoding"] = "identity"  # prevent any compression => we want to know the real size of the file
...
def _get_metadata_or_catch_error(
    *,
    ...
    url = REMOVEDhub_url(repo_id, filename, repo_type=repo_type, revision=revision, endpoint=endpoint)
    ...
    if not local_files_only:
        try:
            try:
                metadata = get_REMOVEDfile_metadata(
...
def _download_to_tmp_and_move(
    ...
    if incomplete_path.exists() and (force_download or (constants.HF_HUB_ENABLE_HF_TRANSFER and not proxies)):
        ...
        # However, if the user has set `force_download=True` or if `REMOVEDtransfer` is enabled, then we should
    ...
    with incomplete_path.open("ab") as f:
        ...
        if xet_file_data is not None and is_xet_available():
            ...
            if xet_file_data is not None and not constants.HF_HUB_DISABLE_XET:
                logger.warning(
                    "Xet Storage is enabled for this repo, but the 'REMOVEDxet' package is not installed. "
                    "Falling back to regular HTTP download. "
                    "For better performance, install the package with: `pip install huggingface_hub[REMOVEDxet]` or `pip install REMOVEDxet`"
                )
