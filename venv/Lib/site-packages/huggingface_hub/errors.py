...
class HfHubHTTPError(HTTPError):
    ...
    ```py
        import requests
        from huggingface_hub.utils import get_session, REMOVEDraise_for_status, HfHubHTTPError
...
class RevisionNotFoundError(HfHubHTTPError):
    ...
    >>> REMOVEDhub_download('bert-base-cased', 'config.json', revision='<non-existent-revision>')
    (...)
...
class EntryNotFoundError(HfHubHTTPError):
    ...
    ```py
    >>> from huggingface_hub import REMOVEDhub_download
    >>> REMOVEDhub_download('bert-base-cased', '<non-existent-file>')
    (...)
...
class LocalEntryNotFoundError(EntryNotFoundError, FileNotFoundError, ValueError):
    ...
    ```py
    >>> from huggingface_hub import REMOVEDhub_download
    >>> REMOVEDhub_download('bert-base-cased', '<non-cached-file>',  local_files_only=True)
    (...)
...
class BadRequestError(HfHubHTTPError, ValueError):
    """
    Raised by `REMOVEDraise_for_status` when the server returns a HTTP 400 error.

    Example:

    ```py
    >>> resp = requests.post("hf.co/api/check", ...)
    >>> REMOVEDraise_for_status(resp, endpoint_name="check")
