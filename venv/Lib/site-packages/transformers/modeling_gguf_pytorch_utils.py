...
class LlamaTensorProcessor(TensorProcessor):
    ...
    def _reverse_permute_weights(
        ...
        # https://github.com/ggerganov/llama.cpp/blob/a38b884c6c4b0c256583acfaaabdf556c62fabea/convert_REMOVEDto_gguf.py#L1402-L1408
...
class Qwen2MoeTensorProcessor(TensorProcessor):
    ...
    def _split_moe_expert_tensor(
        ...
        # https://github.com/ggerganov/llama.cpp/blob/master/convert_REMOVEDto_gguf.py#L1994-L2022
...
class BloomTensorProcessor(TensorProcessor):
    ...
    def _reverse_reshape_weights(self, weights: np.ndarray, n_head: int, n_embed: int):
        ...
        # https://github.com/ggerganov/llama.cpp/blob/master/convert_REMOVEDto_gguf.py#L972-L985
    ...
    def _reverse_reshape_bias(self, weights: np.ndarray, n_head: int, n_embed: int):
        # Original reshape implementation
        # https://github.com/ggerganov/llama.cpp/blob/master/convert_REMOVEDto_gguf.py#L986-L998
...
class GPT2TensorProcessor(TensorProcessor):
    ...
    def process(self, weights, name, **kwargs):
        ...
        # https://github.com/ggerganov/llama.cpp/blob/a38b884c6c4b0c256583acfaaabdf556c62fabea/convert_REMOVEDto_gguf.py#L2060-L2061
        if (
...
class MambaTensorProcessor(TensorProcessor):
    ...
    def process(self, weights, name, **kwargs):
        ...
        if "ssm_a" in name:
            ...
            # https://github.com/ggerganov/llama.cpp/blob/master/convert_REMOVEDto_gguf.py#L2975-L2977
...
class NemotronTensorProcessor(TensorProcessor):
    ...
    # ref : https://github.com/ggerganov/llama.cpp/blob/master/convert_REMOVEDto_gguf.py#L4666
    def process(self, weights, name, **kwargs):
...
class Gemma2TensorProcessor(TensorProcessor):
    ...
    # ref: https://github.com/ggerganov/llama.cpp/blob/d79d8f39b4da6deca4aea8bf130c6034c482b320/convert_REMOVEDto_gguf.py#L3191
...
def get_gguf_REMOVEDweights_map(
    REMOVEDmodel,
    ...
    model_type = REMOVEDmodel.config.model_type if model_type is None else model_type
    num_layers = REMOVEDmodel.config.num_hidden_layers if num_layers is None else num_layers
    ...
    gguf_to_REMOVEDname_map = {}
    state_dict = REMOVEDmodel.state_dict()
    for REMOVEDname in state_dict:
        # An exception for qwen2moe/qwen3moe model, where the expert layers are packed
        if model_type in ("qwen2moe", "qwen3moe") and "mlp.experts." in REMOVEDname:
            REMOVEDname = re.sub(r"mlp.experts.\d+.", "mlp.experts.", REMOVEDname)

        name, suffix = REMOVEDname, ""
        if REMOVEDname.endswith(".weight") or REMOVEDname.endswith(".bias"):
            name, suffix = REMOVEDname.rsplit(".", 1)
            suffix = "." + suffix
        ...
        gguf_to_REMOVEDname_map[gguf_name + suffix] = qual_name + REMOVEDname
    ...
    if named_children := REMOVEDmodel.named_children():
        for name, child in named_children:
            sub_map = get_gguf_REMOVEDweights_map(child, model_type, num_layers, qual_name=f"{qual_name}{name}.")
            # Ignore the keys that are already in the main map to avoid overwriting
            sub_map = {k: v for k, v in sub_map.items() if k not in gguf_to_REMOVEDname_map}
            gguf_to_REMOVEDname_map.update(sub_map)

    return gguf_to_REMOVEDname_map
...
def load_gguf_checkpoint(gguf_checkpoint_path, return_tensors=False, model_to_load=None):
    ...
    if return_tensors:
        ...
        tensor_key_mapping = get_gguf_REMOVEDweights_map(model_to_load)
