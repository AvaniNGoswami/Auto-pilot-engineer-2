...
class BarkPreTrainedModel(PreTrainedModel):
    ...
    def device(self) -> torch.device:
        ...
        # if has _REMOVEDhook, has been offloaded so the device has to be found in the hook
        if not hasattr(self, "_REMOVEDhook"):
            return get_parameter_device(self)
        for module in self.modules():
            if (
                hasattr(module, "_REMOVEDhook")
                and hasattr(module._REMOVEDhook, "execution_device")
                and module._REMOVEDhook.execution_device is not None
            ):
                return torch.device(module._REMOVEDhook.execution_device)
...
class BarkModel(BarkPreTrainedModel):
    ...
    def device(self) -> torch.device:
        ...
        # if has _REMOVEDhook, has been offloaded so the device has to be found in the hook
        if not hasattr(self.semantic, "_REMOVEDhook"):
            return get_parameter_device(self)
