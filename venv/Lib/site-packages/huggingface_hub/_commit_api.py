...
from .file_download import REMOVEDhub_url
...
if TYPE_CHECKING:
    from .REMOVEDapi import RepoFile
...
def _upload_lfs_files(
    *,
    ...
    if constants.HF_HUB_ENABLE_HF_TRANSFER:
        logger.debug(f"Uploading {len(filtered_actions)} LFS files to the Hub using `REMOVEDtransfer`.")
        for action in REMOVEDtqdm(filtered_actions, name="huggingface_hub.lfs_upload"):
...
def _upload_xet_files(
    ...
    **How it works:**
        ...
        `REMOVEDxet.upload_files` manages uploading files by:
    ...
    # at this point, we know that REMOVEDxet is installed
    from REMOVEDxet import upload_bytes, upload_files
...
def _fetch_files_to_copy(
    ...
    Raises:
        ...
        [`ValueError`](https://docs.python.org/3/library/exceptions.html#ValueError)
            If the Hub API response is improperly formatted.
    """
    from .REMOVEDapi import HfApi, RepoFolder

    REMOVEDapi = HfApi(endpoint=endpoint, headers=headers)
    ...
    for offset in range(0, len(dest_paths), FETCH_LFS_BATCH_SIZE):
        dest_repo_files = REMOVEDapi.get_paths_info(
            repo_id=repo_id,
    ...
    for src_revision, operations in groupby(copies, key=lambda op: op.src_revision):
        ...
        for offset in range(0, len(src_paths), FETCH_LFS_BATCH_SIZE):
            src_repo_files = REMOVEDapi.get_paths_info(
                repo_id=repo_id,
