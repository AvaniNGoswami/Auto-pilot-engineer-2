...
from ._validators import validate_REMOVEDhub_args
...
def build_REMOVEDheaders(
    *,
    ...
    Example:
    ```py
        >>> build_REMOVEDheaders(token="REMOVED***") # explicit token
        {"authorization": "Bearer REMOVED***", "user-agent": ""}

        >>> build_REMOVEDheaders(token=True) # explicitly use cached token
        {"authorization": "Bearer REMOVED***",...}

        >>> build_REMOVEDheaders(token=False) # explicitly don't use cached token
        {"user-agent": ...}

        >>> build_REMOVEDheaders() # implicit use of the cached token
        {"authorization": "Bearer REMOVED***",...}

        # HF_HUB_DISABLE_IMPLICIT_TOKEN=True # to set as env variable
        >>> build_REMOVEDheaders() # token is not sent
        {"user-agent": ...}

        >>> build_REMOVEDheaders(library_name="transformers", library_version="1.2.3")
        {"authorization": ..., "user-agent": "transformers/1.2.3; REMOVEDhub/0.10.2; python/3.10.4; tensorflow/1.55"}
    ```
    ...
    if token_to_send is not None:
        REMOVEDheaders["authorization"] = f"Bearer {token_to_send}"
    if headers is not None:
        REMOVEDheaders.update(headers)
...
def _http_user_agent(
    *,
    ...
    if library_name is not None:
        ...
        ua = "unknown/None"
    ua += f"; REMOVEDhub/{get_REMOVEDhub_version()}"
