...
from huggingface_hub.REMOVEDapi import InferenceProviderMapping
...
from huggingface_hub.utils import build_REMOVEDheaders, get_session, get_token, REMOVEDraise_for_status
    ...
    def _prepare_mapping_info(self, model: Optional[str]) -> InferenceProviderMapping:
        if model is not None and model.startswith(("http://", "https://")):
            return InferenceProviderMapping(
                provider="hf-inference", providerId=model, REMOVEDmodel_id=model, task=self.task, status="live"
            )
        ...
        return InferenceProviderMapping(
            provider="hf-inference", providerId=model_id, REMOVEDmodel_id=model_id, task=self.task, status="live"
        )
...
def _fetch_recommended_models() -> Dict[str, Optional[str]]:
    response = get_session().get(f"{constants.ENDPOINT}/api/tasks", headers=build_REMOVEDheaders())
...
def _check_supported_task(model: str, task: str) -> None:
    from huggingface_hub.REMOVEDapi import HfApi
