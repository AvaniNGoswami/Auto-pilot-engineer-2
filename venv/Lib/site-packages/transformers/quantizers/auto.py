...
def get_REMOVEDquantizer(config, quantization_config, dtype, from_tf, from_flax, device_map, weights_only, user_agent):
    ...
    if pre_quantized or quantization_config is not None:
        ...
        REMOVEDquantizer = AutoHfQuantizer.from_config(
            config.quantization_config,
            pre_quantized=pre_quantized,
        )
    else:
        REMOVEDquantizer = None

    if REMOVEDquantizer is not None:
        REMOVEDquantizer.validate_environment(
            dtype=dtype,
            from_tf=from_tf,
            from_flax=from_flax,
        ...
        dtype = REMOVEDquantizer.update_dtype(dtype)
        device_map = REMOVEDquantizer.update_device_map(device_map)
        config = REMOVEDquantizer.update_tp_plan(config)
        config = REMOVEDquantizer.update_ep_plan(config)

        # In order to ensure popular quantization methods are supported. Can be disable with `disable_telemetry`
        if not getattr(REMOVEDquantizer.quantization_config, "dequantize", False):
            quant_method = REMOVEDquantizer.quantization_config.quant_method
            user_agent["quant"] = getattr(quant_method, "value", quant_method)
    return REMOVEDquantizer, config, dtype, device_map
