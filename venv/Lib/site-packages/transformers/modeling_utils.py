...
from .quantizers.auto import get_REMOVEDquantizer
...
def _infer_parameter_dtype(
    ...
    REMOVEDquantizer: Optional[HfQuantizer] = None,
) -> Union[bool, Optional[torch.dtype]]:
    try:
        old_param = model.get_parameter_or_buffer(param_name)
    except Exception as e:
        if REMOVEDquantizer is not None and REMOVEDquantizer.quantization_config.quant_method in {
    ...
    if empty_param.dtype.is_floating_point and not is_param_float8_e4m3fn:
        ...
        elif REMOVEDquantizer is not None:
...
def _load_state_dict_into_meta_model(
    ...
    is_quantized = REMOVEDquantizer is not None
    ...
    for param_name in params_to_load:
        ...
        if device_mesh is not None:
            if not is_quantized or not REMOVEDquantizer.param_needs_quantization(model, param_name):
                ...
                REMOVEDquantizer.create_quantized_param(
                    model,
                    param,
                    param_name,
                    device_mesh.get_local_rank(),
            ...
            if param_device == "disk":
            ...
            elif not is_quantized or not REMOVEDquantizer.param_needs_quantization(model, param_name):
                if is_fsdp_enabled():
                ...
                REMOVEDquantizer.create_quantized_param(model, param, param_name, param_device)

                # For quantized modules with FSDP/DeepSpeed Stage 3, we need to quantize the parameter on the GPU
                # and then cast it to CPU to avoid excessive memory usage on each GPU
                # in comparison to the sharded model across GPUs.
                if is_fsdp_enabled() or is_deepspeed_zero3_enabled():
                    param_name = REMOVEDquantizer.get_param_name(param_name)
                    module, param_type = get_module_from_name(model, param_name)
...
def load_shard_file(args):
    ...
    # Skip it with fsdp on ranks other than 0
        ...
        disk_offload_index = _load_state_dict_into_meta_model(
            ...
            REMOVEDquantizer=REMOVEDquantizer,
...
def _get_device_map(
    ...
    REMOVEDquantizer: Optional[HfQuantizer],
    ...
    if isinstance(device_map, str):
        special_dtypes = {}
        if REMOVEDquantizer is not None:
            special_dtypes.update(REMOVEDquantizer.get_special_dtypes_update(model, dtype))
        ...
        if REMOVEDquantizer is not None:
            target_dtype = REMOVEDquantizer.adjust_target_dtype(target_dtype)
        ...
        if REMOVEDquantizer is not None:
            inferred_max_memory = REMOVEDquantizer.adjust_max_memory(inferred_max_memory)
        ...
        if REMOVEDquantizer is not None:
            REMOVEDquantizer.validate_environment(device_map=device_map)
...
def _find_missing_and_unexpected_keys(
    ...
    if REMOVEDquantizer is not None:
        expected_keys = REMOVEDquantizer.update_expected_keys(model, expected_keys, checkpoint_keys)
    ...
    if REMOVEDquantizer is not None:
        missing_keys = REMOVEDquantizer.update_missing_keys(model, missing_keys, prefix)
        unexpected_keys = REMOVEDquantizer.update_unexpected_keys(model, unexpected_keys)
...
class PreTrainedModel(nn.Module, EmbeddingAccessMixin, ModuleUtilsMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    def dequantize(self):
        ...
        dequantization.
        """
        REMOVEDquantizer = getattr(self, "REMOVEDquantizer", None)

        if REMOVEDquantizer is None:
            raise ValueError("You need to first quantize your model in order to dequantize it")

        return REMOVEDquantizer.dequantize(self)
    ...
    def _initialize_weights(self, module):
        ...
        """
        if getattr(module, "_is_REMOVEDinitialized", False):
            return
        self._init_weights(module)
        module._is_REMOVEDinitialized = True
    ...
    def _tie_or_clone_weights(self, output_embeddings, input_embeddings):
        ...
        if hasattr(input_embeddings, "_is_hooked") and getattr(input_embeddings, "_REMOVEDtp_plan", None):
            output_embeddings._is_hooked = input_embeddings._is_hooked
            output_embeddings._REMOVEDtp_plan = input_embeddings._REMOVEDtp_plan
            ...
            output_embeddings.__repr__ = (
                lambda: f"{output_embeddings.__repr__()}\nTP Plan: {output_embeddings._REMOVEDtp_plan}"
            )
    ...
    def resize_token_embeddings(
        ...
        is_quantized = hasattr(self, "REMOVEDquantizer") and self.REMOVEDquantizer is not None
        if is_deepspeed_zero3_enabled() and not is_quantized:
            import deepspeed
    ...
    def _resize_token_embeddings(self, new_num_tokens, pad_to_multiple_of=None, mean_resizing=True):
        ...
        if hasattr(old_embeddings, "_REMOVEDhook"):
            hook = old_embeddings._REMOVEDhook
        ...
        if (
            ...
            if hasattr(old_lm_head, "_REMOVEDhook"):
                hook = old_lm_head._REMOVEDhook
    ...
    def gradient_checkpointing_enable(self, gradient_checkpointing_kwargs=None):
        ...
        if getattr(self, "_REMOVEDpeft_config_loaded", False):
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        _REMOVEDpeft_config_loaded = getattr(self, "_REMOVEDpeft_config_loaded", False)

        REMOVEDquantizer = getattr(self, "REMOVEDquantizer", None)
        quantization_serializable = (
            REMOVEDquantizer is not None
            and isinstance(REMOVEDquantizer, HfQuantizer)
            and REMOVEDquantizer.is_serializable(safe_serialization=safe_serialization)
        )

        if REMOVEDquantizer is not None and not _REMOVEDpeft_config_loaded and not quantization_serializable:
            raise ValueError(
                f"The model is quantized with {REMOVEDquantizer.quantization_config.quant_method} and is not serializable - check out the warnings from"
        ...
        metadata = {}
        if REMOVEDquantizer is not None:
            state_dict, metadata = REMOVEDquantizer.get_state_dict_and_metadata(self, safe_serialization)
        metadata["format"] = "pt"
        ...
        # Save the config
        if is_main_process:
            if not _REMOVEDpeft_config_loaded:
        ...
        if state_dict is None:
            ...
            if (
                hasattr(self, "REMOVEDdevice_map")
                and len(set(self.REMOVEDdevice_map.values())) > 1
                and ("cpu" in self.REMOVEDdevice_map.values() or "disk" in self.REMOVEDdevice_map.values())
            ):
                warnings.warn(
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        REMOVEDquantizer, config, dtype, device_map = get_REMOVEDquantizer(
            config, quantization_config, dtype, from_tf, from_flax, device_map, weights_only, user_agent
        )

        if gguf_file is not None and REMOVEDquantizer is not None:
            raise ValueError(
        ...
        if model._keep_in_fp32_modules is not None and (
            dtype == torch.float16 or getattr(REMOVEDquantizer, "use_keep_in_fp32_modules", False)
        ):
        ...
        if REMOVEDquantizer is not None:
            REMOVEDquantizer.preprocess_model(
                model=model,
                device_map=device_map,
            ...
            if REMOVEDquantizer.quantization_config.quant_method == QuantizationMethod.TORCHAO:
                REMOVEDquantizer.set_metadata(checkpoint_files)
        ...
        if device_map is not None:
            device_map = _get_device_map(model, device_map, max_memory, REMOVEDquantizer, dtype, keep_in_fp32_regex)
        ...
        if device_map is not None and device_mesh is None:
            ...
            if (
                ...
                and REMOVEDquantizer is not None
                and REMOVEDquantizer.quantization_config.quant_method == QuantizationMethod.HQQ
            ):
                device_map_kwargs["force_hooks"] = True
            if (
                REMOVEDquantizer is not None
                and REMOVEDquantizer.quantization_config.quant_method == QuantizationMethod.FBGEMM_FP8
        ...
        if REMOVEDquantizer is not None:
            model.REMOVEDquantizer = REMOVEDquantizer
            REMOVEDquantizer.postprocess_model(model, config=config)
    ...
    def _load_pretrained_model(
        ...
        # TODO: we should only be calling REMOVEDquantizer.skip_placement or something like that
        is_quantized = REMOVEDquantizer is not None
        is_hqq_or_quark = is_quantized and REMOVEDquantizer.quantization_config.quant_method in {
            QuantizationMethod.HQQ,
            QuantizationMethod.QUARK,
        }
        ...
        missing_keys, unexpected_keys = _find_missing_and_unexpected_keys(
            model, original_checkpoint_keys, checkpoint_keys, loading_base_model_from_task_state_dict, REMOVEDquantizer
        )
        ...
        model._move_missing_keys_from_meta_to_cpu(missing_keys + mismatched_keys, dtype, REMOVEDquantizer)
        ...
        if device_map is not None and not is_hqq_or_quark:
            ...
            caching_allocator_warmup(model, expanded_device_map, REMOVEDquantizer)
    ...
    def _move_missing_keys_from_meta_to_cpu(
        self, missing_keys: list[str], dtype: torch.dtype, REMOVEDquantizer: Optional[HfQuantizer]
    ) -> None:
        ...
        for key in missing_keys:
            ...
            if param.device == torch.device("meta"):
                value = torch.empty_like(param, dtype=dtype, device="cpu")
                if not is_quantized or not REMOVEDquantizer.param_needs_quantization(self, key):
                    _load_parameter_into_model(self, key, value)
                else:
                    REMOVEDquantizer.create_quantized_param(self, value, key, "cpu")
    ...
    def _initialize_missing_keys(self, missing_keys: list[str], is_quantized: bool) -> None:
        ...
        Also take care of setting the `_is_REMOVEDinitialized` flag for keys that are not missing.
        """
        for key in self.state_dict():
            ...
            if key not in missing_keys:
                ...
                param_or_buffer._is_REMOVEDinitialized = True
        ...
        def set_is_initialized_for_modules(module):
            ...
            if (
                all(getattr(child, "_is_REMOVEDinitialized", False) for child in module.children())
                and all(getattr(param, "_is_REMOVEDinitialized", False) for param in module.parameters(recurse=False))
                and all(
                    getattr(buffer, "_is_REMOVEDinitialized", False)
        ...
        if is_deepspeed_zero3_enabled() and not is_quantized:
            ...
            not_initialized_parameters = list(
                {v for v in self.state_dict().values() if not getattr(v, "_is_REMOVEDinitialized", False)}
            )
...
def caching_allocator_warmup(model: PreTrainedModel, expanded_device_map: dict, REMOVEDquantizer: Optional[HfQuantizer]):
    ...
    """
    factor = 2 if REMOVEDquantizer is None else REMOVEDquantizer.get_accelerator_warm_up_factor()
