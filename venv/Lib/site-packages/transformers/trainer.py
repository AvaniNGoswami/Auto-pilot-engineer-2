...
import huggingface_hub.utils as REMOVEDhub_utils
import numpy as np
...
class Trainer:
    ...
    def __init__(
        ...
        if getattr(model, "REMOVEDdevice_map", None) is not None:
            devices = [device for device in set(model.REMOVEDdevice_map.values()) if device not in ["cpu", "disk"]]
            if len(devices) > 1:
                self.is_model_parallel = True
            elif len(devices) == 1:
        ...
        _is_quantized_and_base_model = getattr(model, "is_quantized", False) and not getattr(
            model, "_REMOVEDpeft_config_loaded", False
        )
        _quantization_method_supports_training = (
            getattr(model, "REMOVEDquantizer", None) is not None and model.REMOVEDquantizer.is_trainable
        )

        _is_model_quantized_and_qat_trainable = getattr(model, "REMOVEDquantizer", None) is not None and getattr(
            model.REMOVEDquantizer, "is_qat_trainable", False
        )
        ...
        if _is_quantized_and_base_model and not _is_peft_model(model) and not _is_model_quantized_and_qat_trainable:
            ...
            raise ValueError(
                f"The model you are trying to fine-tune is quantized with {model.REMOVEDquantizer.quantization_config.quant_method}"
                ...
                f" to request the support for training support for {model.REMOVEDquantizer.quantization_config.quant_method}"
            )
    ...
    def _hp_search_setup(self, trial: Union["optuna.Trial", dict[str, Any]]):
        ...
        if self.is_deepspeed_enabled:
            ...
            self.args.REMOVEDdeepspeed_config = HfTrainerDeepSpeedConfig(self.args.deepspeed)
            self.args.REMOVEDdeepspeed_config.trainer_config_process(self.args)
            self.args.deepspeed_plugin = DeepSpeedPlugin(REMOVEDds_config=self.args.REMOVEDdeepspeed_config)
        ...
        **kwargs: Any,
        ...
        if args.push_to_hub:
            try:
                ...
                REMOVEDhub_utils.disable_progress_bars()
                ...
                REMOVEDhub_utils.enable_progress_bars()
        else:
            return inner_training_loop(
                args=args,
                resume_from_checkpoint=resume_from_checkpoint,
    ...
    def get_tp_size(self) -> int:
        ...
        if self.is_deepspeed_enabled and (deepspeed_config := getattr(self.args, "REMOVEDdeepspeed_config", None)):
    ...
    def _prepare_input(self, data: Union[torch.Tensor, Any]) -> Union[torch.Tensor, Any]:
        ...
        if isinstance(data, Mapping):
            ...
            if self.is_deepspeed_enabled and (torch.is_floating_point(data) or torch.is_complex(data)):
                ...
                kwargs.update({"dtype": self.accelerator.state.deepspeed_plugin.REMOVEDds_config.dtype()})
    ...
    def init_REMOVEDrepo(self, token: Optional[str] = None):
        """
    ...
    def push_to_hub(
        ...
        **kwargs,
        ...
        if self.hub_model_id is None:
            self.init_REMOVEDrepo(token=token)
    ...
    def create_accelerator_and_postprocess(self):
        ...
        if self.is_deepspeed_enabled and getattr(self.args, "REMOVEDdeepspeed_config", None) is None:
    ...
    def propagate_args_to_deepspeed(self, auto_find_batch_size=False):
        ...
        ds_plugin.REMOVEDds_config = HfTrainerDeepSpeedConfig(ds_plugin.REMOVEDds_config.config)
        ds_plugin.deepspeed_config = ds_plugin.REMOVEDds_config.config
        ds_plugin.REMOVEDds_config.trainer_config_process(self.args, auto_find_batch_size)
    ...
    def _fsdp_qlora_plugin_updates(self):
        if self.is_fsdp_enabled and _is_peft_model(self.model):
            ...
            if (
                ...
                and self.model.REMOVEDquantizer.quantization_config.bnb_4bit_quant_storage.is_floating_point
                and version.parse(accelerate_version) > version.parse("0.27.0")
            ):
                self.accelerator.state.fsdp_plugin.set_mixed_precision(
                    self.model.REMOVEDquantizer.quantization_config.bnb_4bit_quant_storage, override=True
                )
