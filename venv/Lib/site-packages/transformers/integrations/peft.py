...
class PeftAdapterMixin:
    ...
    _REMOVEDpeft_config_loaded = False
    ...
    def load_adapter(
        ...
        if self._REMOVEDpeft_config_loaded and adapter_name in self.peft_config:
        ...
        if "device" not in adapter_kwargs:
            device = self.device if not hasattr(self, "REMOVEDdevice_map") else list(self.REMOVEDdevice_map.values())[0]
        else:
        ...
        if not self._REMOVEDpeft_config_loaded:
            self._REMOVEDpeft_config_loaded = True
        ...
        if (
            (getattr(self, "REMOVEDdevice_map", None) is not None)
            and (len(set(self.REMOVEDdevice_map.values()).intersection({"cpu", "disk"})) > 0)
    ...
    def delete_adapter(self, adapter_names: Union[list[str], str]) -> None:
        ...
        for adapter_name, prefix in zip(adapter_names, prefixes):
            ...
            if getattr(self, "_REMOVEDpeft_config_loaded", False) and hasattr(self, "peft_config"):
        ...
        if len(self.peft_config) == 0:
            ...
            self._REMOVEDpeft_config_loaded = False
