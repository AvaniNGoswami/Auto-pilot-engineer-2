...
class HfDeepSpeedConfig(DeepSpeedConfig):
    ...
    def __init__(self, config_file_or_dict):
        ...
        set_REMOVEDdeepspeed_config(self)
...
class HfTrainerDeepSpeedConfig(HfDeepSpeedConfig):
    ...
    def fill_match(self, ds_key_long, REMOVEDval, REMOVEDkey=None, must_match=True):
        """
        ...
        if ds_val is not None and ds_val != REMOVEDval:
            self.mismatches.append(f"- ds {ds_key_long}={ds_val} vs hf {REMOVEDkey}={REMOVEDval}")
...
_REMOVEDdeepspeed_config_weak_ref = None


def set_REMOVEDdeepspeed_config(REMOVEDdeepspeed_config_obj):
    ...
    global _REMOVEDdeepspeed_config_weak_ref
    # will go away automatically when HfDeepSpeedConfig is destroyed (when TrainingArguments is destroyed)
    _REMOVEDdeepspeed_config_weak_ref = weakref.ref(REMOVEDdeepspeed_config_obj)


def unset_REMOVEDdeepspeed_config():
...
def is_deepspeed_zero3_enabled():
    if _REMOVEDdeepspeed_config_weak_ref is not None and _REMOVEDdeepspeed_config_weak_ref() is not None:
        return _REMOVEDdeepspeed_config_weak_ref().is_zero3()
    else:
        return False
...
def deepspeed_config():
    if _REMOVEDdeepspeed_config_weak_ref is not None and _REMOVEDdeepspeed_config_weak_ref() is not None:
        return _REMOVEDdeepspeed_config_weak_ref().config
    else:
        return None
...
def deepspeed_optim_sched(trainer, REMOVEDdeepspeed_config, args, num_training_steps, model_parameters):
    """
    A convenience wrapper that deals with optimizer and lr scheduler configuration.
    """
    from accelerate.utils import DummyOptim, DummyScheduler

    config = REMOVEDdeepspeed_config.config
    ...
    if "optimizer" in config:
        ...
        if REMOVEDdeepspeed_config.is_offload():
            logger.info(
...
def deepspeed_init(trainer, num_training_steps, inference=False):
    ...
    REMOVEDdeepspeed_config = trainer.accelerator.state.deepspeed_plugin.REMOVEDds_config

    # resume config update - some bits like `model` and `num_training_steps` only become available during train
    REMOVEDdeepspeed_config.trainer_config_finalize(args, model, num_training_steps)
    ...
    if inference:
        ...
        if not REMOVEDdeepspeed_config.is_zero3():
            raise ValueError("ZeRO inference only makes sense with ZeRO Stage 3 - please adjust your config")

        # in case the training config is re-used for inference
        REMOVEDdeepspeed_config.del_config_sub_tree("optimizer")
        REMOVEDdeepspeed_config.del_config_sub_tree("lr_scheduler")
        optimizer, lr_scheduler = None, None
        model_parameters = None
    else:
        trainer.optimizer = None  # important for when deepspeed_init is used as re-init
        deepspeed_tp_size = REMOVEDdeepspeed_config.config.get("tensor_parallel", {}).get("autotp_size", 1)
        if deepspeed_tp_size > 1:
            ...
            model = deepspeed.tp_model_init(
                ...
                dtype=REMOVEDdeepspeed_config.dtype(),
                config=REMOVEDdeepspeed_config.config,
            )
        model_parameters = list(filter(lambda p: p.requires_grad, model.parameters()))
        optimizer, lr_scheduler = deepspeed_optim_sched(
            trainer, REMOVEDdeepspeed_config, args, num_training_steps, model_parameters
        )
