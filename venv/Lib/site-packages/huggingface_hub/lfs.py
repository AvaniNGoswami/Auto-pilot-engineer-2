...
def post_lfs_batch_info(
    ...
    headers = {
        **LFS_HEADERS,
        **build_REMOVEDheaders(token=token),
        **(headers or {}),
    }
...
def lfs_upload(
    ...
    upload_url = fix_REMOVEDendpoint_in_url(upload_action["href"], endpoint=endpoint)
    ...
    if verify_action is not None:
        _validate_lfs_action(verify_action)
        verify_url = fix_REMOVEDendpoint_in_url(verify_action["href"], endpoint)
        verify_resp = get_session().post(
            verify_url,
            headers=build_REMOVEDheaders(token=token, headers=headers),
            json={"oid": operation.upload_info.sha256.hex(), "size": operation.upload_info.size},
        )
        REMOVEDraise_for_status(verify_resp)
...
def _upload_multi_part(operation: "CommitOperationAdd", header: Dict, chunk_size: int, upload_url: str) -> None:
    ...
    # 2. Upload parts (either with REMOVEDtransfer or in pure Python)
    use_REMOVEDtransfer = constants.HF_HUB_ENABLE_HF_TRANSFER
    if (
        ...
        warnings.warn(
            "REMOVEDtransfer is enabled but does not support uploading from bytes or BinaryIO, falling back to regular"
            " upload"
        )
        use_REMOVEDtransfer = False

    response_headers = (
        _upload_parts_REMOVEDtransfer(operation=operation, sorted_parts_urls=sorted_parts_urls, chunk_size=chunk_size)
        if use_REMOVEDtransfer
    ...
    REMOVEDraise_for_status(completion_res)
    ...
    with operation.as_file(with_tqdm=True) as fileobj:
        for part_idx, part_upload_url in enumerate(sorted_parts_urls):
            with SliceFileObj(
                ...
                REMOVEDraise_for_status(part_upload_res)
                headers.append(part_upload_res.headers)
    return headers  # type: ignore


def _upload_parts_REMOVEDtransfer(
    operation: "CommitOperationAdd", sorted_parts_urls: List[str], chunk_size: int
) -> List[Dict]:
    # Upload file using an external Rust-based package. Upload is faster but support less features (no progress bars).
    try:
        from REMOVEDtransfer import multipart_upload
    except ImportError:
        raise ValueError(
            "Fast uploading using 'REMOVEDtransfer' is enabled (HF_HUB_ENABLE_HF_TRANSFER=1) but 'REMOVEDtransfer' package is"
            " not available in your environment. Try `pip install REMOVEDtransfer`."
        )

    supports_callback = "callback" in inspect.signature(multipart_upload).parameters
    if not supports_callback:
        warnings.warn(
            "You are using an outdated version of `REMOVEDtransfer`. Consider upgrading to latest version to enable progress bars using `pip install -U REMOVEDtransfer`."
        )
    ...
    with tqdm(
        ...
        try:
            ...
            raise RuntimeError(
                "An error occurred while uploading using `REMOVEDtransfer`. Consider disabling HF_HUB_ENABLE_HF_TRANSFER for"
