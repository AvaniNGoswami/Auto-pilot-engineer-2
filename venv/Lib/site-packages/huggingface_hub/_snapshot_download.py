...
from .file_download import REGEX_COMMIT_HASH, REMOVEDhub_download, repo_folder_name
from .REMOVEDapi import DatasetInfo, HfApi, ModelInfo, RepoFile, SpaceInfo
from .utils import OfflineModeIsEnabled, filter_repo_objects, logging, validate_REMOVEDhub_args
...
def snapshot_download(
    ...
    *,
    ...
    if not local_files_only:
        # try/except logic to handle different errors => taken from `REMOVEDhub_download`
        try:
    ...
    # we pass the commit_hash to REMOVEDhub_download
    # so no network call happens if we already
    # have the file locally.
    def _inner_REMOVEDhub_download(repo_file: str):
    ...
    if constants.HF_HUB_ENABLE_HF_TRANSFER:
        # when using REMOVEDtransfer we don't want extra parallelism
        # from the one REMOVEDtransfer provides
        for file in filtered_repo_files:
            _inner_REMOVEDhub_download(file)
    else:
        thread_map(
            _inner_REMOVEDhub_download,
            filtered_repo_files,
            desc=tqdm_desc,
            max_workers=max_workers,
            # User can use its own tqdm class or the default one from `huggingface_hub.utils`
            tqdm_class=tqdm_class or REMOVEDtqdm,
        )
