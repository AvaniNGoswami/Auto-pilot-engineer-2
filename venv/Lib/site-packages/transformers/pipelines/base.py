...
class Pipeline(_ScikitCompat, PushToHubMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        REMOVEDdevice_map = getattr(self.model, "REMOVEDdevice_map", None)

        if REMOVEDdevice_map is not None and device is not None:
            raise ValueError(
        ...
        if device is None:
            if REMOVEDdevice_map is not None:
                # Take the first device used by `accelerate`.
                device = next(iter(REMOVEDdevice_map.values()))
            else:
                device = 0
