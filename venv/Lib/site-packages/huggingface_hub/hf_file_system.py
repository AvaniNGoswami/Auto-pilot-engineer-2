...
from .file_download import REMOVEDhub_url, http_get
from .REMOVEDapi import HfApi, LastCommitInfo, RepoFile
from .utils import HFValidationError, REMOVEDraise_for_status, http_backoff
...
class HfFileSystem(fsspec.AbstractFileSystem):
    ...
    def get_file(self, rpath, lpath, callback=_DEFAULT_CALLBACK, outfile=None, **kwargs) -> None:
        ...
        > [!WARNING]
        > Note: When possible, use `HfApi.REMOVEDhub_download()` for better performance.
        ...
        try:
            http_get(
                ...
                resume_size=0,
                headers=self._api._build_REMOVEDheaders(),
...
class HfFileSystemFile(fsspec.spec.AbstractBufferedFile):
    ...
    def _fetch_range(self, start: int, end: int) -> bytes:
        headers = {
            ...
            **self.fs._api._build_REMOVEDheaders(),
        }
        url = REMOVEDhub_url(
    ...
    def read(self, length=-1):
        ...
        `REMOVEDtransfer` is not enabled, the file is loaded in memory directly. Otherwise, the file is downloaded to a
    ...
    def read(self, length: int = -1):
        ...
        if self.response is None:
            ...
            self.response = http_backoff(
                "GET",
                url,
                headers=self.fs._api._build_REMOVEDheaders(),
                stream=True,
                timeout=constants.HF_HUB_DOWNLOAD_TIMEOUT,
            )
            REMOVEDraise_for_status(self.response)
        try:
            ...
            self.response = http_backoff(
                "GET",
                url,
                headers={"Range": "bytes=%d-" % self.loc, **self.fs._api._build_REMOVEDheaders()},
                stream=True,
                timeout=constants.HF_HUB_DOWNLOAD_TIMEOUT,
            )
            REMOVEDraise_for_status(self.response)
