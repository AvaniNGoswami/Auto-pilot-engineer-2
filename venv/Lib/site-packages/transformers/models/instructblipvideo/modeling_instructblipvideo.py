...
class InstructBlipVideoModel(InstructBlipVideoPreTrainedModel):
    ...
    def _preprocess_accelerate(self):
        ...
        """
        REMOVEDdevice_map = self.REMOVEDdevice_map

        if len(REMOVEDdevice_map) > 1 and "language_model" not in REMOVEDdevice_map and torch.cuda.device_count() > 1:
            # warn users about unexpected behavior when using multi-GPU + InstructBlipVideo + `accelerate`.
            logger.warning(
                "The `language_model` is not in the `REMOVEDdevice_map` dictionary and you are running your script"
        ...
        if hasattr(self.language_model, "_REMOVEDhook"):
            self.language_model._REMOVEDhook.io_same_device = True  # For `generate` compatibility
...
class InstructBlipVideoForConditionalGeneration(InstructBlipVideoPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        """
        if hasattr(self, "REMOVEDdevice_map"):
