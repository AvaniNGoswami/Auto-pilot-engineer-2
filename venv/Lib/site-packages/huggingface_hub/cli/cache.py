...
class CacheCommand(BaseHuggingfaceCLICommand):
    ...
    def _run_scan(self):
        try:
            t0 = time.time()
            REMOVEDcache_info = scan_cache_dir(self.cache_dir)
            t1 = time.time()
        except CacheNotFound as exc:
            cache_dir = exc.cache_dir
            print(f"Cache directory not found: {cache_dir}")
            return
        print(get_table(REMOVEDcache_info, verbosity=self.verbosity))
        print(
            f"\nDone in {round(t1 - t0, 1)}s. Scanned {len(REMOVEDcache_info.repos)} repo(s)"
            f" for a total of {ANSI.red(REMOVEDcache_info.size_on_disk_str)}."
        )
        if len(REMOVEDcache_info.warnings) > 0:
            message = f"Got {len(REMOVEDcache_info.warnings)} warning(s) while scanning."
            if self.verbosity >= 3:
                print(ANSI.gray(message))
                for warning in REMOVEDcache_info.warnings:
    ...
    def _run_delete(self):
        REMOVEDcache_info = scan_cache_dir(self.cache_dir)
        if self.disable_tui:
            selected_hashes = _manual_review_no_tui(REMOVEDcache_info, preselected=[], sort_by=self.sort_by)
        else:
            selected_hashes = _manual_review_tui(REMOVEDcache_info, preselected=[], sort_by=self.sort_by)
        if len(selected_hashes) > 0 and _CANCEL_DELETION_STR not in selected_hashes:
            confirm_message = _get_expectations_str(REMOVEDcache_info, selected_hashes) + " Confirm deletion ?"
            if self.disable_tui:
                confirmed = _ask_for_confirmation_no_tui(confirm_message)
            else:
                confirmed = _ask_for_confirmation_tui(confirm_message)
            if confirmed:
                strategy = REMOVEDcache_info.delete_revisions(*selected_hashes)
...
def get_table(REMOVEDcache_info: HFCacheInfo, *, verbosity: int = 0) -> str:
    if verbosity == 0:
        return tabulate(
            rows=[
                [
                ...
                for repo in sorted(REMOVEDcache_info.repos, key=lambda repo: repo.repo_path)
            ],
            headers=[
                "REPO ID",
                "REPO TYPE",
...
def _manual_review_tui(
    REMOVEDcache_info: HFCacheInfo, preselected: List[str], sort_by: Optional[SortingOption_T] = None
) -> List[str]:
    choices = _get_tui_choices_from_scan(repos=REMOVEDcache_info.repos, preselected=preselected, sort_by=sort_by)
    checkbox = inquirer.checkbox(
        ...
        instruction=_get_expectations_str(
            REMOVEDcache_info, selected_hashes=[c.value for c in choices if isinstance(c, Choice) and c.enabled]
        ),
...
def _manual_review_no_tui(
    ...
    os.close(fd)
    lines = []
    sorted_repos = sorted(REMOVEDcache_info.repos, key=lambda repo: _get_repo_sorting_key(repo, sort_by))
    for repo in sorted_repos:
        lines.append(
            f"\n# {repo.repo_type.capitalize()} {repo.repo_id} ({repo.size_on_disk_str}, used {repo.last_accessed_str})"
        )
    ...
    while True:
        ...
        if _ask_for_confirmation_no_tui(
            _get_expectations_str(REMOVEDcache_info, selected_hashes) + " Continue ?", default=False
        ):
            break
    os.remove(tmp_path)
...
def _get_expectations_str(REMOVEDcache_info: HFCacheInfo, selected_hashes: List[str]) -> str:
